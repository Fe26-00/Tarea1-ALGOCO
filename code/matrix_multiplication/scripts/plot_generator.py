...
# Para que cada algoritmo tenga siempre el mismo color
COLORES_ALGORITMOS = {
    "naive": "#1f77b4",
    "strassen": "#ff7f0e",
}
...
def cargar_datos():
    # Leemos las mediciones separadas por espacios
...
def promediar_por_tamano(df, columna_y):
    ...
    # Si hay una sola medición, std queda como NaN
...
def graficar_lineas(df, columna_y, etiqueta_y, titulo, nombre_archivo):
    ...
    for algoritmo, grupo in resumen.groupby("algoritmo"):
        ...
        # Dejamos una banda para ver cuánto varían las mediciones
    ...
    # Usamos escala logarítmica para ambos ejes
    ax.set_xscale("log")
    ax.set_yscale("log")
...
def graficar_barras_por_dominio(df):
    ...
    for i, dominio in enumerate(dominios):
        ...
        for algoritmo in algoritmos:
            ...
            # Si no hay datos para ese caso, ponemos 0
            valores.append(
    ...
    # Centramos las etiquetas debajo de los grupos
    ax.set_xticks(
...
def graficar_complejidad_teorica(df):
    ...
    # Usamos el tamaño más grande para ajustar las curvas
    n_max = resumen["tamano"].max()
    n_valores = np.sort(resumen["tamano"].unique()).astype(float)

    referencias = {
        "O(n^3)": n_valores ** 3,
        "O(n^2.807) [Strassen]": n_valores ** 2.807,
    }

    for nombre_ref, valores in referencias.items():
        # Ajustamos la curva al rango de los datos
    ...
    ax.set_title(
        "Comparación de curvas empíricas con complejidad teórica de referencia"
    )
...
def main():
    ...
    graficar_lineas(
        ...
        "Dimensión de la matriz vs. memoria utilizada promedio",
    ...
    # Comparación con las complejidades esperadas
